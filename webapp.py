...
# HTML Templates (mantener los mismos)
INDEX_HTML = """
<!DOCTYPE html>
<html lang="es">
<head>
...
# Rutas Flask (mantener las mismas pero actualizar la función de búsqueda)
@app.route('/')
def index():
    return INDEX_HTML
...
def test_endpoint():
    ...
    return jsonify({{
        'status': 'SUCCESS',
        'message': '🇺🇸 Price Finder USA con búsqueda específica está funcionando!',
        'timestamp': datetime.now().isoformat(),
        'version': '2.0 - Específica'
    }})

if __name__ == '__main__':
    port = int(os.environ.get('PORT', 5000))
    app.run(host='0.0.0.0', port=port, debug=False)d: #3b82f6; color: white; border: none; padding: 16px 24px; 
            border-radius: 12px; cursor: pointer; font-size: 16px; font-weight: 600; 
